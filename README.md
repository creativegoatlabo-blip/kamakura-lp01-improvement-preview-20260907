...
セクションの表示時に見出しや写真を印象づけたいという依頼に合わせ、深緑・木・生成りのデザインとPico CSSはそのままに、スクロール演出を追加しました。

- 見出しは行ごとに下から現れ、セクションラベルには細い罫線が伸びます。
- 写真には小さな拡大と浮き上がり、制作事例・基本情報・制作ステップには時間差をつけています。
- 各要素の初回表示時だけ動き、戻りスクロールで再び消えることはありません。スクロールの固定・乗っ取りは行いません。
- `IntersectionObserver` とCSSの `transform` / `opacity` で実装。新しい外部ライブラリは追加せず、`motion.css` と `motion.js` の合計は約4.9KBです。
- 端末の「視差効果を減らす」設定、途中での同設定変更、印刷に対応。JavaScriptなし・演出用JSの読み込み失敗時も本文は表示されます。フォームの送信防止処理は演出と別ファイルに維持しています。
- 予約ボタン・店舗選択・FAQ・入力フォームには表示待ちの演出を加えていません。

検証: HTML・JS構文、320 / 375 / 414 / 768 / 1440pxで全48対象の表示完了、行ごとの時間差、戻りスクロールでの再実行なし、横はみ出し・画像欠落なし。モーション抑制・JS無効・演出JS失敗・印刷時のフォールバック、予約位置への移動、店舗選択、確認ダイアログ、FAQ、外部POSTなしを確認済み。
